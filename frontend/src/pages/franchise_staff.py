...
def _build_id_cards_pdf(staff_rows: list[dict], current_user: User) -> bytes:
    ...
    styles.add(ParagraphStyle(name='CardTitle', parent=styles['Normal'], fontSize=10.5, leading=11, textColor=colors.HexColor('#6d28d9'), spaceAfter=0))
    styles.add(ParagraphStyle(name='CardSub', parent=styles['Normal'], fontSize=4.8, leading=5.4, textColor=colors.HexColor('#6b6478')))
    styles.add(ParagraphStyle(name='NameBig', parent=styles['Normal'], fontSize=9.2, leading=9.8, textColor=colors.HexColor('#1f1630'), spaceAfter=0))
    styles.add(ParagraphStyle(name='Small', parent=styles['Normal'], fontSize=5.5, leading=6.5, textColor=colors.HexColor('#6b6478'), spaceBefore=2))
    styles.add(ParagraphStyle(name='Tiny', parent=styles['Normal'], fontSize=5.2, leading=5.1, textColor=colors.HexColor('#6b6478'), alignment=1))
    styles.add(ParagraphStyle(name='Footer', parent=styles['Normal'], fontSize=6, leading=5.8, textColor=colors.white, alignment=1))
    styles.add(ParagraphStyle(name='PageSmall', parent=styles['Normal'], fontSize=7, leading=8))

    story = [
        Paragraph('Staff ID Cards', styles['Title']),
        Paragraph(f'Generated by {escape(current_user.full_name or current_user.email or "System Administrator")}', styles['PageSmall']),
        Spacer(1, 4*mm),
    ]
    ...
    def card(row):
        ...
        photo = _photo_flowable(row.get('profile_photo'), width_mm=18, height_mm=20)
        if photo is None:
            photo = Paragraph('<b>ID</b><br/><font size="4">PHOTO</font>', styles['Tiny'])

        logo = _logo_flowable(28, 10) or Paragraph('<b>LOGO</b>', styles['Tiny'])
        ...
        role_badge = Table([
            [Paragraph(f"<font color='white'><b>🌐 {_fit_text(role_label, 16)}</b></font>", styles['Tiny'])]
        ], colWidths=[24*mm], rowHeights=[5.5*mm])
        role_badge.setStyle(TableStyle([
            ('BACKGROUND', (0,0), (-1,-1), colors.HexColor('#6d28d9')),
            ('BOX', (0,0), (-1,-1), 0, colors.HexColor('#6d28d9')),
        ...
        info = [
            Paragraph(f"<b>{escape(_fit_text(first_name, 18))}</b>", styles['NameBig']),
            Paragraph(f"<b>{escape(_fit_text(surname, 18))}</b>", styles['NameBig']) if surname else Paragraph('', styles['Small']),
            role_badge,
            Paragraph(f"▦ Franchise: {escape(_fit_text(str(franchise_name), 24))}", styles['Small']),
        ]

        qr_box = Table([[qr_drawing]], colWidths=[19*mm], rowHeights=[19*mm])
        qr_box.setStyle(TableStyle([
            ('BOX', (0,0), (-1,-1), 0.8, colors.HexColor('#6d28d9')),
        ...
        body = Table([[photo, info, qr_box]], colWidths=[23*mm, 25*mm, 26*mm], rowHeights=[24*mm])
        ...
        meta = Table([
            [
                Paragraph('<b>Status</b><br/><font color="#15803d">Active</font>', styles['Tiny']),
                Paragraph(f'<b>User ID</b><br/>{escape(str(user_id))}', styles['Tiny']),
                Paragraph('<b>Issued</b><br/>21 May 2025', styles['Tiny']),
                Paragraph('<b>ID Validity</b><br/>No Expiry', styles['Tiny']),
            ]
        ], colWidths=[17.5*mm, 17.5*mm, 17.5*mm, 17.5*mm], rowHeights=[7*mm])
        meta.setStyle(TableStyle([
            ('LINEABOVE', (0,0), (-1,0), 0.35, colors.HexColor('#eee6fb')),
            ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
            ('ALIGN', (0,0), (-1,-1), 'CENTER'),
            ('LEFTPADDING', (0,0), (-1,-1), 0),
            ('RIGHTPADDING', (0,0), (-1,-1), 0),
        ...
        footer = Table([
            [Paragraph(f"Scan QR code or visit {escape(_fit_text(qr_payload, 52))}", styles['Footer'])]
        ], colWidths=[74*mm], rowHeights=[5*mm])
        ...
        inner = Table([[header], [body], [meta], [footer]], colWidths=[inner_w], rowHeights=[10*mm, 22*mm, 7*mm, 5*mm])
        ...
        outer.setStyle(TableStyle([
            ('BOX', (0,0), (-1,-1), 1.0, colors.HexColor('#6d28d9')),
            ('BACKGROUND', (0,0), (-1,-1), colors.white),
            ('LEFTPADDING', (0,0), (-1,-1), safe_margin),
            ('RIGHTPADDING', (0,0), (-1,-1), safe_margin),
            ('TOPPADDING', (0,0), (-1,-1), safe_margin),
            ('BOTTOMPADDING', (0,0), (-1,-1), safe_margin),
        ]))
        return outer
    ...
    grid = Table(
        ...
        rowHeights=[54*mm] * len(grid_rows),
        hAlign='CENTER'
    )
...
def delete_manager(manager_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    write_audit_log(db, actor_user_id=current_user.id, action="deactivate", entity_type="manager", entity_id=manager_id, franchise_user_id=row["franchise_user_id"], old_values=dict(row), new_values={"is_active": False}, note="Manager deactivated")
    db.commit()
    return {"message": "Manager made inactive", "manager_id": manager_id}
...
def delete_employee(employee_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    db.execute(text("UPDATE users SET is_active = FALSE, updated_at = :now WHERE id = :user_id"), {"user_id": row["user_id"], "now": now})
    write_audit_log(db, actor_user_id=current_user.id, action="deactivate", entity_type="employee", entity_id=employee_id, franchise_user_id=row["franchise_user_id"], old_values=dict(row), new_values={"is_active": False}, note="Employee deactivated")
    db.commit()
    return {"message": "Employee made inactive", "employee_id": employee_id}

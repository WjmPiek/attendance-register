...
import {
  ...
  getCommissionEntryHistory, getCommissionEntry,
} from '../api/client'
...
export default function CommissionPage({ me }) {
  ...
  useEffect(()=>{load()},[])
  useEffect(()=>()=>{if(reviewPdfUrl)URL.revokeObjectURL(reviewPdfUrl)},[reviewPdfUrl])
  ...
  const linkedEmployees=useMemo(()=>employees.filter(x=>x.staff_type==='employee'),[employees])
  const staffHierarchy=useMemo(()=>{
    const q=employeeSearch.trim().toLowerCase()
    const matches=item=>[item.full_name,item.employee_number,item.employee_role,item.email,item.office_address_assigned].some(v=>String(v||'').toLowerCase().includes(q))
    const managers=employees.filter(item=>item.staff_type==='manager')
    const assignedIds=new Set()
    const groups=managers.map(manager=>{
      const children=employees.filter(item=>item.staff_type==='employee'&&Number(item.manager_user_id)===Number(manager.manager_profile_id))
      children.forEach(item=>assignedIds.add(Number(item.user_id)))
      const visibleChildren=q?children.filter(matches):children
      if(q&&!matches(manager)&&!visibleChildren.length)return null
      return {manager,employees:visibleChildren}
    }).filter(Boolean)
    const direct=employees.filter(item=>item.staff_type==='employee'&&!assignedIds.has(Number(item.user_id))&&(!q||matches(item)))
  ...
  const closeReview=()=>{setEditing(null);if(reviewPdfUrl)URL.revokeObjectURL(reviewPdfUrl);setReviewPdfUrl('')}
  const openReview=async entry=>{
    try{
      setError('');setLoading(true)
      const blob=await downloadCommissionForm(entry.id)
      if(reviewPdfUrl)URL.revokeObjectURL(reviewPdfUrl)
      setReviewPdfUrl(URL.createObjectURL(blob));setEditing({...entry})
    }catch(x){setError(x.message||'Commission review document could not load.')}finally{setLoading(false)}
  }
  const review=async status=>{try{await reviewCommissionEntry(editing.id,{...payload(editing),status,review_notes:editing.review_notes||''});closeReview();setNotice(`Commission ${status}. Employee notification sent.`);await refresh(filter)}catch(x){setError(x.message)}}
  ...
  useEffect(()=>{
    const id=Number(sessionStorage.getItem('commissionFocusId')||0)
    if(!id||isSuper||!employees.length)return
    let activeRequest=true
    ;(async()=>{
      try{
        const entry=await getCommissionEntry(id)
        if(!entry||!activeRequest)return
        const employee=employees.find(item=>Number(item.user_id)===Number(entry.employee_user_id))
        if(isFranchiseScope&&employee)await openEmployee(employee)
        if(!activeRequest)return
        sessionStorage.removeItem('commissionFocusId')
        await openReview(entry)
      }catch(x){
        if(activeRequest)setError(x.message||'The commission notification could not be opened.')
      }
    })()
    return()=>{activeRequest=false}
  },[employees])

  const staffButton=(employee,nested=false)=><button type="button" className={`commission-employee-row${nested?' nested':''}`} key={employee.user_id} onClick={()=>openEmployee(employee)}><div><strong>{employee.full_name}</strong><span>{employee.employee_role||'Employee'}{employee.employee_number?` · ${employee.employee_number}`:''}</span>{employee.office_address_assigned&&<small>{employee.office_address_assigned}</small>}</div><span className="commission-open-label">Open commissions →</span></button>
  const viewingLinkedEmployee=isManager&&selectedEmployee?.staff_type==='employee'

  if(editing){
    return <section className="stack-page commission-review-page"><div className="commission-review-workspace"><div className="section-heading"><div><button type="button" className="glass-button" onClick={closeReview}>← Back to commissions</button><p className="eyebrow">Approval document</p><h1>Review commission #{editing.id}</h1><p>{editing.employee_name} · {editing.reference}</p></div></div>{error&&<div className="error-message">{error}</div>}{reviewPdfUrl?<div className="pdf-preview commission-review-pdf"><iframe src={reviewPdfUrl} title={`Commission review ${editing.id}`}/></div>:<p>Preparing review PDF...</p>}<div className="commission-review-decision"><label>Decision notes<textarea placeholder="Add an approval or rejection note" value={editing.review_notes||''} onChange={e=>setEditing(v=>({...v,review_notes:e.target.value}))}/></label><div className="button-row"><button className="primary-button" onClick={()=>review('approved')}>Approve claim</button><button className="danger-button" onClick={()=>review('rejected')}>Reject claim</button></div></div></div></section>
  }

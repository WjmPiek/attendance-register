...
export default function CommissionPage({ me }) {
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      try{
        const result=await getCommissionEntries({})
        const entry=result.items.find(item=>Number(item.id)===id)
  ...
  return <section className="stack-page">
    ...
    {editing&&<div className="commission-review-workspace"><div className="section-heading"><div><p className="eyebrow">Approval document</p><h2>Review commission #{editing.id}</h2><p>{editing.employee_name} · {editing.reference}</p></div><button className="glass-button" onClick={closeReview}>Close</button></div>{reviewPdfUrl?<div className="pdf-preview commission-review-pdf"><iframe src={reviewPdfUrl} title={`Commission review ${editing.id}`}/></div>:<p>Preparing review PDF...</p>}<div className="commission-review-decision"><label>Decision notes<textarea placeholder="Add an approval or rejection note" value={editing.review_notes||''} onChange={e=>setEditing(v=>({...v,review_notes:e.target.value}))}/></label><div className="button-row"><button className="primary-button" onClick={()=>review('approved')}>Approve claim</button><button className="danger-button" onClick={()=>review('rejected')}>Reject claim</button></div></div></div>}

...
export default function MobileAttendancePage({ me, onDone }) {
    ...
    try {
      ...
      while (Date.now() < deadline) {
        ...
        if (codes.length) {
          ...
          captureCameraPhoto()
          setMessage('Office QR scanned, linked and attendance photo captured.')
          stopCamera()
          return
        }
  ...
  const handleAction = async (action) => {
    ...
    try {
      ...
      const payload = {
        ...
        attendance_photo: attendancePhoto || null,
      }
  ...
  return (
    <Card title="Mobile Employee Attendance">
      ...
      <div className="qr-scan-card">
        <div className="detail-header mobile-qr-header">
          <div>
            ...
            <p className="muted small">QR is optional. Scan it when available; GPS coordinates and signature are always required. A photo is captured automatically after a successful scan.</p>
          </div>
        ...
        {attendancePhoto ? <img src={attendancePhoto} alt="Attendance capture" className="attendance-capture-preview" /> : null}
      ...
      <p className="muted small">GPS coordinates and signature are compulsory. QR is optional. Outside-office events are flagged and notifications are sent for review. On-road events go to approval review.</p>

...
export default function CommissionPage({ me }) {
  ...
  if(isFranchise && !selectedEmployee){
    return <section className="stack-page">
      <div className="glass-card section-card"><div className="section-heading"><div><h1>Staff Commissions</h1><p>Select an employee to open a dedicated commission page and add or review their commissions.</p></div></div>{error&&<div className="error-message">{error}</div>}{notice&&<div className="success-message">{notice}</div>}</div>
      {structurePanel}

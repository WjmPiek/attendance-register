...
PRESERVE_IF_BLANK_ON_EDIT = {"id_number", "username", "password", "employee_number", "email"}
...
def _require_staff_manager(db: Session, user: User) -> int | None:
    names = set(_roles(db, user))
    if "SuperUser" in names:
        return None
    if "FranchiseUser" in names:
        return _franchise_profile_id(db, user.id)
    raise HTTPException(status_code=403, detail="Only SuperUser or FranchiseUser can manage staff")
...
def _staff_franchise_id(
    ...
    current_franchise_id = _require_staff_manager(db, current_user)
    if current_franchise_id is not None:
        if requested_franchise_user_id is not None and int(requested_franchise_user_id) != int(current_franchise_id):
            raise HTTPException(status_code=403, detail="Selected franchise is outside your scope")
        return int(current_franchise_id)
...
def _preserve_blank_identity_values(values: dict) -> dict:
    cleaned = dict(values)
    for field in PRESERVE_IF_BLANK_ON_EDIT:
        if field in cleaned and isinstance(cleaned[field], str) and not cleaned[field].strip():
            cleaned.pop(field)
    return cleaned
...
def _ensure_user_exists(db: Session, user_id: int) -> None:
    ...
    """), {"user_id": user_id}).mappings().first()
    if not row:
        raise HTTPException(status_code=400, detail="Staff member must be linked to an active user account")


def _ensure_user_role(db: Session, user_id: int, role_name: str) -> None:
    row = db.execute(text("""
        SELECT 1
        FROM user_roles ur
        ...
        LIMIT 1
    """), {"user_id": user_id, "role_name": role_name}).first()
    if not row:
        raise HTTPException(status_code=400, detail=f"Staff user must have the {role_name} role")


def _ensure_franchise_exists(db: Session, franchise_user_id: int) -> None:
    row = db.execute(text("""
        SELECT id
        FROM franchise_users
        WHERE id = :franchise_user_id
          AND COALESCE(is_active, TRUE) = TRUE
        LIMIT 1
    """), {"franchise_user_id": franchise_user_id}).mappings().first()
    if not row:
        raise HTTPException(status_code=400, detail="Staff member must be linked to an active franchise")


def _ensure_manager_assignment(db: Session, manager_user_id: int | None, franchise_user_id: int) -> None:
    ...
    manager = db.execute(text("""
        ...
        WHERE id = :manager_user_id
          ...
          AND COALESCE(is_active, TRUE) = TRUE
        LIMIT 1
    """), {"manager_user_id": manager_user_id, "franchise_user_id": franchise_user_id}).mappings().first()
    if not manager:
        raise HTTPException(status_code=400, detail="Selected manager is not under this franchise")


def _ensure_staff_integrity(
    db: Session,
    *,
    staff_type: str,
    user_id: int,
...
def _unique_username(db: Session, base: str, exclude_user_id: int | None = None) -> str:
...
def _ensure_staff_identity_unique(
    db: Session,
    franchise_user_id: int,
    employee_number: str | None = None,
    id_number: str | None = None,
    ...
    employee_number = (employee_number or "").strip() or None
    ...
    if duplicate:
        ...
        raise HTTPException(status_code=400, detail="Employee number already belongs to active staff")
...
def _scope_staff_row(db: Session, current_user: User, staff_type: str, staff_id: int):
    ...
    if current_franchise_id is not None:
        scope_sql = "AND s.franchise_user_id = :franchise_user_id"
        params["franchise_user_id"] = current_franchise_id
    row = db.execute(text(f"""
...
def list_managers(
    ...
    if _is_superuser(db, current_user):
        ...
        if franchise_user_id is not None:
            where = "AND mu.franchise_user_id = :franchise_user_id"
            params["franchise_user_id"] = franchise_user_id
        rows = db.execute(text(f"""
            SELECT
                mu.id,
        ...
        if "FranchiseUser" in names:
            franchise_id = _franchise_profile_id(db, current_user.id)
            rows = db.execute(text("""
                SELECT
                    mu.id,
                    mu.user_id,
                ...
                WHERE mu.franchise_user_id = :franchise_user_id
                  ...
                  AND COALESCE(u.is_active, TRUE) = TRUE
                ORDER BY mu.id DESC
            """), {"franchise_user_id": franchise_id}).mappings().all()
...
def list_employees(
    ...
    if _is_superuser(db, current_user):
        ...
        params = {}
        if franchise_user_id is not None:
            where = "AND eu.franchise_user_id = :franchise_user_id"
            params["franchise_user_id"] = franchise_user_id
        rows = db.execute(text(f"""
            SELECT
                eu.id,
...
def _create_user(db: Session, full_name: str, email: str | None, password: str, role_name: str, username: str | None = None):
    ...
    normalised_username = username.strip().lower() if username else None
    if not normalised_email and not normalised_username:
        raise HTTPException(status_code=400, detail="Email or username is required")

    role = db.query(Role).filter(Role.name == role_name).first()
    if not role:
        raise HTTPException(status_code=500, detail=f"{role_name} role not found")

    conflicts = db.query(User).filter(or_(
        User.email == normalised_email if normalised_email else False,
        User.username == normalised_username if normalised_username else False,
    )).all()
    for existing in conflicts:
        ...
        if getattr(existing, "email", None):
            existing.email = f"{archived_suffix}_{existing.email}"[:250]
...
def create_manager(payload: CreateManagerRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    _ensure_office_hours_columns(db)
    franchise_user_id = _staff_franchise_id(db, current_user, payload.franchise_user_id)
    if payload.office_area_id is None:
        raise HTTPException(status_code=400, detail="Select a registered franchise office")
    ...
    login_email = _ensure_email_available(db, str(payload.email)) if payload.email else None
...
def create_employee(payload: CreateEmployeeRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    _ensure_manager_assignment(db, payload.manager_user_id, franchise_user_id)
...
def _staff_scope_filter(db: Session, current_user: User, table: str, staff_id: int):
    ...
    current_franchise_id = None if _is_superuser(db, current_user) else (
        _franchise_profile_id(db, current_user.id) if "FranchiseUser" in set(_roles(db, current_user)) else None
    )
    if _is_superuser(db, current_user) or current_franchise_id is not None:
        scope_sql = ""
        params = {"staff_id": staff_id}
        if current_franchise_id is not None:
            scope_sql = "AND s.franchise_user_id = :franchise_user_id"
...
def update_manager(manager_id: int, payload: UpdateManagerRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    values = _preserve_blank_identity_values({k: v for k, v in payload.model_dump(exclude_unset=True).items() if v is not None})
    _ensure_staff_identity_unique(
        db,
        row["franchise_user_id"],
        employee_number=values.get("employee_number"),
        id_number=values.get("id_number"),
        exclude_user_id=row["user_id"],
    )
    if values.get("email"):
        values["email"] = _ensure_email_available(db, str(values["email"]), row["user_id"])
    ...
    if "name" in values or "surname" in values or "email" in values or "username" in values or "is_active" in values or "password" in values:
        ...
        if values.get("email"):
            ...
            user_params["email"] = values["email"]
        if values.get("username"):
            user_updates.append("username = :username")
            user_params["username"] = _unique_username(db, str(values["username"]), row["user_id"])
    ...
    if "office_area_id" in values:
        _assign_office_gps(db, row["user_id"], values.get("office_area_id"), row["franchise_user_id"])
    _ensure_staff_integrity(
        db,
        staff_type="manager",
        user_id=row["user_id"],
...
def update_employee(employee_id: int, payload: UpdateEmployeeRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    values = _preserve_blank_identity_values({k: v for k, v in payload.model_dump(exclude_unset=True).items() if v is not None})
    if "manager_user_id" in payload.model_fields_set and payload.manager_user_id is None:
    ...
    if values.get("employee_role") and values["employee_role"] not in EMPLOYEE_ROLES:
        raise HTTPException(status_code=400, detail="Invalid employee role")
    _ensure_manager_assignment(db, values.get("manager_user_id"), row["franchise_user_id"])
    updates = []
